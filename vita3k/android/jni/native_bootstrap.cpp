...
JNIEXPORT jboolean JNICALL
Java_org_vita3k_emulator_NativeLib_reinitialize(JNIEnv *env, jclass, jstring storage_path_str) {
    auto &session = android_session_state();
    if (has_game_running()) {
        LOG_WARN("Cannot reinitialize Vita3K Android while a game is running.");
        return JNI_FALSE;
    }
    ...
    LOG_INFO("Vita3K Android reinitialised.");
    return JNI_TRUE;
}
