...
void reset_perf_metrics(EmuEnvState &emuenv) {
    ...
    renderer.perf_fps = 0;
    renderer.perf_avg_fps = 0;
    renderer.perf_min_fps = 0;
    renderer.perf_max_fps = 0;
    renderer.perf_ms_per_frame = 0;
    std::fill_n(renderer.perf_fps_values, perf_frames_size, 0.0f);
    renderer.perf_fps_values_count = 0;
    renderer.perf_current_fps_offset = 0;
}
...
void sync_perf_overlay_config(EmuEnvState &emuenv) {
    ...
    auto &renderer = *emuenv.renderer;
    renderer.performance_overlay = emuenv.cfg.performance_overlay;
    renderer.performance_overlay_position = emuenv.cfg.performance_overlay_position;
    renderer.performance_overlay_detail = emuenv.cfg.performance_overlay_detail;
}
...
FirmwareState get_firmware_state(const EmuEnvState &emuenv) {
    return FirmwareState{
        .pd0 = has_installed_firmware_content(emuenv.pref_path / "pd0"),
        .vs0 = has_installed_firmware_content(emuenv.pref_path / "vs0"),
        .sa0 = has_installed_firmware_content(emuenv.pref_path / "sa0"),
    };
}
...
void prepare_game_launch_overlay(EmuEnvState &emuenv) {
    ...
    renderer.precompile_progress.store(0, std::memory_order_relaxed);
    renderer.precompile_requested.store(false, std::memory_order_relaxed);
    ...
    if (renderer::get_shaders_cache_hashs(renderer) && emuenv.cfg.shader_cache) {
        ...
        renderer.precompile_progress.store(0, std::memory_order_relaxed);
        renderer.precompile_complete.store(false, std::memory_order_relaxed);
        renderer.precompile_requested.store(true, std::memory_order_release);
    }
}
...
void update_runtime_metrics(EmuEnvState &emuenv, LaunchRuntimeMetrics &metrics) {
    ...
    auto &renderer = *emuenv.renderer;
    renderer.perf_fps = emuenv.fps;
    renderer.perf_avg_fps = emuenv.avg_fps;
    renderer.perf_min_fps = emuenv.min_fps;
    renderer.perf_max_fps = emuenv.max_fps;
    renderer.perf_ms_per_frame = emuenv.ms_per_frame;
    std::copy(std::begin(emuenv.fps_values), std::end(emuenv.fps_values), std::begin(renderer.perf_fps_values));
    renderer.perf_fps_values_count = perf_frames_size;
    renderer.perf_current_fps_offset = emuenv.current_fps_offset;
}
