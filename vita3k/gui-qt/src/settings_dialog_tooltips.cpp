...
SettingsDialogTooltips::SettingsDialogTooltips(QObject *parent)
    ...
    , fps_hack(tr("Game hack. May double the framerate from 30 FPS to 60 FPS in some games,\nbut can cause some games to run twice as fast."))
    ...
    , audio_backend(tr("Select the audio backend.\nCubeb is recommended for most systems."))
    ...
    , check_for_updates(tr("Automatically check for updates at startup."))
    ...
    , validation_layer(tr("Enable Vulkan validation layers.\nUseful for debugging but reduces performance."))
