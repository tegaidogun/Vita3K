...
static void render_loop(renderer::State &state, DisplayState &display, GxmState &gxm, MemState &mem, Config &config) {
    ...
    if (state.precompile_requested.load(std::memory_order_acquire)) {
        ...
        for (int i = 0; i < total && !state.render_abort.load(std::memory_order_relaxed); ++i) {
            ...
            state.precompile_progress.store(i + 1, std::memory_order_release);
    ...
    if (!state.precompile_requested.load(std::memory_order_relaxed)
